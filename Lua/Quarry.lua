...
local function saveProgress() --Session persistence
...
local a = 1; if facing == 2 then a = -1 end
...
-------------------------------------------------------------------------------------
if facing ~= 0 then
if xPos > 1 then turnTo(2) else turnTo(0) end
end
...
if not restoreFound then mine(false,false, true) else if turtle.digUp() and turtle.digDown() then mined = mined + 2 end end
...
layersDone = layersDone + 3; end ---------------Height End-------
if lastHeight ~= 0 then ---------LAST ROW--------- (copied from above)
moved = moved + 1
if y ~= 0 then down(2) end --If the basic y == 2 or 1
rowCheck = "right"
if rowCheck == "right" then relxPos = xPos else relxPos = (x-xPos)+1 end
while zPos <= z do -------------Width----------
while relxPos < x do ------------Length---------
